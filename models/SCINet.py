...
class LevelSCINet(nn.Module):
    def __init__(self, attention_d, in_planes, attention_module, kernel_size, dropout, groups, hidden_size, INN):
        super(LevelSCINet, self).__init__()
        self.attention_module = attention_module
        if self.attention_module == 'Self':
            self.attention = ScaledDotProductAttention(d_model=attention_d, d_k=attention_d, d_v=attention_d, h=8,
                                                       dropout=0.5)
        elif self.attention_module == 'CBAM':
            self.attention = CBAMBlock(channel=attention_d, reduction=16, kernel_size=kernel_size)
...
class SCINet_Tree(nn.Module):
    def __init__(self, input_len, in_planes, attention_module, num_levels, current_level, kernel_size, dropout, groups,
                 hidden_size, INN):
        super().__init__()
        self.current_level = current_level

        self.attention_d = int(input_len / math.pow(2, num_levels - current_level - 1))
        ...
        if current_level != 0:
            self.SCINet_Tree_odd = SCINet_Tree(input_len, in_planes, attention_module, num_levels, current_level - 1,
                                               kernel_size,
                                               dropout, groups, hidden_size, INN)
            self.SCINet_Tree_even = SCINet_Tree(input_len, in_planes, attention_module, num_levels, current_level - 1,
                                                kernel_size,
                                                dropout, groups, hidden_size, INN)
...
class EncoderTree(nn.Module):
    def __init__(self, input_len, in_planes, attention_module, num_levels, kernel_size, dropout, groups, hidden_size,
                 INN):
        super().__init__()
